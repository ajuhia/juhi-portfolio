          ...
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              <div key={index} className="relative flex items-start animate-fade-in">
                {/* Timeline dot */}
                <div className="absolute left-6 w-4 h-4 bg-steel-blue-900 border-4 border-steel-blue-900 rounded-full"></div>

                {/* Content */}
                <div className="ml-16 bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 w-full border-l-4 border-steel-blue-900">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-gray-900 font-poppins">{exp.title}</h3>
                      <h4 className="text-lg font-semibold text-maroon-900">{exp.company}</h4>
                    </div>
                    <div className="text-right">
                      ...
                      <div className={`text-xs font-medium px-2 py-1 rounded-full mt-1 ${
                        exp.type === 'Internship' 
                          ? 'bg-maroon-900/10 text-maroon-900' 
                          : 'bg-steel-blue-900/10 text-steel-blue-900'
                      }`}>
                        {exp.type}
                      </div>
                    </div>
